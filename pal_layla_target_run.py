...
PAL_RUN = os.environ.get("PAL_RUN", "layla_only")
...
# This script always early-stops BOTH stages (unlike the parent script, where stage 1 is a
# fixed epoch budget) -- qasr_lev is the target here, not an out-of-domain pretrain corpus,
# so there is no "budget-matched" comparison to preserve by fixing stage 1's epoch count.
...
DATASET_DIR = "/root/Palestinian-ASR/data_layla_v1"
...
STAGE1_BY_RUN = {
    ...
    "run9_qasr_lev_masc_lev_nonlev50h_ep2":
        "/root/Palestinian-ASR/data_stage1_v1/qasr_lev_masc_lev_plus_nonlev50h",
    # qasr_lev-as-target study: stage 1 = qasr_lev(train only) + casa_pal(all splits) +
    # casa_jor(all) + omni(all) + layla(all), validated against qasr_lev's own val split
    # (DATASET_DIR now points at data_qasrlev_v1, so SPLITS/eval_both are qasr_lev-native
    # throughout -- see scripts/build_stage1_qasrlev_target.py). Stage 2 fine-tunes further
    # on qasr_lev train ONLY (SPLITS["train"]), same val/test targets.
    "run12_qasrlev_target": "/root/Palestinian-ASR/data_stage1_v1/qasrlev_target_mix",
    # ---- Layla-as-target transfer study ----
    # DATASET_DIR now points at data_layla_v1 (Layla's own native train/val/test), so SPLITS/
    # eval_both throughout this script are Layla-native. Every entry below except "layla_only"
    # is meant to be launched with PAL_STAGE1_CKPT set to that run's existing pretrained
    # checkpoint (see the resolver-derived paths in run_layla_target_chain.sh) -- stage 1 is
    # never retrained, only its already-merged weights are reused as the base for a FRESH
    # LoRA stage fine-tuned on Layla instead of Palestinian. STAGE1_DIR here is only used for
    # accurate stage1_hours/STAGE1_DIR bookkeeping in the summary, same convention as
    # run2_jor_s1e1of2 etc. above.
    "layla_only": None,  # no pretrain at all -- single-stage LoRA straight on layla train
    "run2_1ep_to_layla": "/root/Palestinian-ASR/data_stage1_v1/jor",
    "run2_2ep_to_layla": "/root/Palestinian-ASR/data_stage1_v1/jor",
    "run3_1ep_to_layla": "/root/Palestinian-ASR/data_stage1_v1/omni",
    "run3_2ep_to_layla": "/root/Palestinian-ASR/data_stage1_v1/omni",
    "run4_1ep_to_layla": "/root/Palestinian-ASR/data_stage1_v1/omni_jor",
    "run4_2ep_to_layla": "/root/Palestinian-ASR/data_stage1_v1/omni_jor",
    "run6_1ep_to_layla": "/root/Palestinian-ASR/data_stage1_v1/qasr_lev_10h",
    "run6_2ep_to_layla": "/root/Palestinian-ASR/data_stage1_v1/qasr_lev_10h",
    "run7_1ep_to_layla": "/root/Palestinian-ASR/data_stage1_v1/qasr_non_lev_50h",
    "run7_2ep_to_layla": "/root/Palestinian-ASR/data_stage1_v1/qasr_non_lev_50h",
    "run8_1ep_to_layla": "/root/Palestinian-ASR/data_stage1_v1/qasr_lev_masc_lev",
    "run8_2ep_to_layla": "/root/Palestinian-ASR/data_stage1_v1/qasr_lev_masc_lev",
    "run9_1ep_to_layla": "/root/Palestinian-ASR/data_stage1_v1/qasr_lev_masc_lev_plus_nonlev50h",
    "run9_2ep_to_layla": "/root/Palestinian-ASR/data_stage1_v1/qasr_lev_masc_lev_plus_nonlev50h",
    # run11's 3-stage pipeline, layla substituted in place of its original stage3 (Palestinian):
    # PAL_STAGE1_CKPT = run11's stage1_merged_adapter, PAL_STAGE1_CKPT2 = stage2_merged_adapter
    # (sequential merge -- see PAL_STAGE1_CKPT2 above). STAGE1_DIR here is just bookkeeping,
    # pointing at run11's actual stage-1 corpus (qasr_non_lev_50h).
    "run11_to_layla": "/root/Palestinian-ASR/data_stage1_v1/qasr_non_lev_50h",
    # Looser-threshold (>0.65, vs the pipeline's original >=0.80) QASR-only Levantine pretrain
    # corpus (see scripts/build_stage1_lev65_layla_target.py) -- launched WITHOUT
    # PAL_STAGE1_CKPT, so stage 1 trains fresh here (to early stopping, validating against
    # Layla's own val since DATASET_DIR=data_layla_v1 throughout this script), then stage 2
    # fine-tunes on Layla train, same as every other entry above.
    "lev65_to_layla": "/root/Palestinian-ASR/data_stage1_v1/lev65_combined",
}
...
# Second optional bypass checkpoint, merged sequentially AFTER PAL_STAGE1_CKPT (not stacked
# independently onto the fresh base) -- needed for run11, whose stage2_merged_adapter's LoRA
# weights were trained on top of an ALREADY stage1-merged model, so merging it alone onto a
# fresh base would reproduce the wrong weights. Only run11's layla-transfer variant sets this.
PAL_STAGE1_CKPT2 = os.environ.get("PAL_STAGE1_CKPT2")
...
if STAGE1_SPLITS is None:
    ...
    if PAL_STAGE1_CKPT:
        ...
        # ---- STAGE 1: qasr_lev(train)+casa_pal+casa_jor+omni+layla, to early stopping ----
        # Unlike the parent script's fixed-epoch stage 1 (there is no "budget-matched"
        # comparison to preserve here -- qasr_lev is the actual target, not an out-of-domain
        # pretrain corpus), this run trains stage 1 the same way run1_pal_only trains its
        # single stage: shared TrainConfigSpec defaults, <=50 epochs, patience 3 on val WER
        # (which, via STAGE1_SPLITS["validation"] == SPLITS["validation"], is qasr_lev's own
        # native val split -- see DATASET_DIR / build_stage1_qasrlev_target.py).
        s1_spec = train_spec
        print(f"\n=== STAGE 1: {STAGE1_DIR} | {len(STAGE1_SPLITS['train'])} rows | "
              f"<=50 epoch(s), early stopping patience 3 (qasr_lev val) ===")
        TRAIN_OUTS["stage1"] = TrainAPI.run(adapter, STAGE1_SPLITS, s1_spec, lora_spec, tag="stage1")
        mlflow.end_run()

        # Restore the best-epoch checkpoint explicitly rather than trusting the in-memory
        # weights (which are whatever epoch training stopped on, not necessarily the best one
        # once early stopping is in play) -- this makes the merge provably operate on
    ...
    if PAL_STAGE1_CKPT2:
        # ---- MERGE #2: sequentially fold a second pretrain stage's adapter in (run11 only) ----
        set_seed()
        adapter.apply_lora(lora_spec)
        adapter.load_checkpoint(Path(PAL_STAGE1_CKPT2))
        print(f"[ckpt] stage1b adapter <- {PAL_STAGE1_CKPT2}")
        assert isinstance(adapter.model, PeftModel), type(adapter.model)
        adapter.model = adapter.model.merge_and_unload()
        adapter.model.config.use_cache = False
        print(f"[merge] stage-1b LoRA merged -> {type(adapter.model).__name__}, "
              f"{sum(p.numel() for p in adapter.model.parameters())/1e6:.1f}M params")
        gc.collect(); torch.cuda.empty_cache() if DEVICE == "cuda" else None
        MERGED_DIR2 = CKPT_DIR / "stage1b_merged_adapter"
        MERGED_DIR2.mkdir(parents=True, exist_ok=True)
        shutil.copytree(PAL_STAGE1_CKPT2, MERGED_DIR2, dirs_exist_ok=True)
        eval_both("stage1b_merged", force=True)

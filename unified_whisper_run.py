...
# 
# One notebook, one registry of adapters for all five model families (OmniASR, FastConformer-CTC,
# Qwen3-ASR, CohereAsr Transcribe, Whisper). **Change `MODEL_NAME` and `DATASET_DIR` in Cell 2 — nothing
...
import sys, datetime
_LOG_DIR = Path("/root/Palestinian-ASR/Runs/whisper_medium_finetune_mix_v2/logs")
...
# DVC-tracked (v1.0.0), materialized to LOCAL disk (not the /workspace network mount)
# specifically so training I/O is fast -- see data_finetune_mix_v2.
DATASET_DIR = "/root/Palestinian-ASR/data_finetune_mix_v2"
# ================================================================
...
_LOCAL_MLFLOW_DIR = Path("/root/Palestinian-ASR/Runs/whisper_medium_finetune_mix_v2")
...
class ConfigAPI:
    ...
    _LORA = {
        ...
        "openai/whisper-medium": LoRAConfigSpec(
            ...
            # for medium) -- identical target_modules, same r=32/alpha=32 blanket default
            # used across every model family here.
            target_modules=["q_proj","k_proj","v_proj","out_proj","fc1","fc2"]),
    }
    _TRAIN = {
        ...
        "openai/whisper-medium": TrainConfigSpec(
            ...
            # waveform-frontend headroom rather than matching it outright).
...
class WhisperAdapter(ModelAdapter):
    ...
    def collate(self, feats):
        ...
        tok = self.processor.tokenizer
        lab = tok([f["text"] for f in feats], padding=True, return_tensors="pt")
...
def _load_split_dataset(split: str):
    files = sorted(_glob.glob(str(REAL_DATA_DIR / split / "*.parquet")))
    if not files:
        raise FileNotFoundError(f"no parquet shards under {REAL_DATA_DIR / split}")

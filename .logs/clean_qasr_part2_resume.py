...
INPUT_ROOT = Path("/workspace/asr/Palestinian-ASR")
OUTPUT_ROOT = Path("/workspace/asr/Palestinian-ASR/data_cleaned_text_qasr_part2_v1")

# Only the QASR recordings that the 961-wav run never covered, segmented into
# processed_qasr_segments_part2/ by preprocess/qasr_segment_to_arrow.py --wav-stem-list.
# The distinct root name keeps stable_file_id() hashes disjoint from the 110 QASR
# shards already merged into data_cleaned_text_merged_v1/, so the merge cannot collide.
# Index JSONL files under index/ are excluded by the glob, same as the original run.
DISCOVERY_GLOBS = [
    "processed_qasr_segments_part2/train/*.arrow",
]
...
# RESUME MODE: the first Step B run died at 283/304 shards on a disk-quota error.
# All 283 written clean shards were verified to have intact parquet footers, so they are kept
# and only the missing inputs are reprocessed. Never rmtree the output root in this variant.
...
# Resume filter: drop inputs whose clean shard already exists AND is a readable parquet file.
# A truncated tail file (quota death mid-write) would fail the footer read and be reprocessed.
_already_done = []
_todo = []
for _p in input_files:
    _clean = CLEAN_DIR / f"{stable_file_id(_p)}__clean.parquet"
    if _clean.exists():
        try:
            pq.ParquetFile(_clean).metadata
            _already_done.append(_p)
            continue
        except Exception as _e:
            print(f"RESUME: reprocessing {_p.name}, existing clean shard unreadable: {_e!r}")
            _clean.unlink()
    _todo.append(_p)

print(f"RESUME: {len(_already_done):,} shards already complete, {len(_todo):,} to process")
input_files = _todo
if not input_files:
    print("RESUME: nothing left to process.")
    raise SystemExit(0)

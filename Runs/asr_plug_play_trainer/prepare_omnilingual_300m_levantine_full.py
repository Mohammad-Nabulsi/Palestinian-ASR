...
def materialize_split(
    ...
    for idx, row in enumerate(source_rows):
        ...
        if (idx + 1) % 100 == 0:
            print(f"[{split}] materialized {idx + 1}/{len(source_rows)}")
...
def main() -> None:
    ...
    config = make_config(smoke_mode=False, num_train_epochs=1, run_baseline_before_train=False, run_post_train_eval=False)

    source_rows = {split: read_jsonl(path) for split, path in SOURCE_MANIFESTS.items()}
    output_rows = {}
    split_summary = {}
    for split, rows in source_rows.items():
        print(f"Materializing split={split} rows={len(rows)}")

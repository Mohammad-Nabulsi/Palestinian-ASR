...
RUN_DIR = REPO_ROOT / "Runs" / "whisper_medium_levantine_custom_streaming_5minckpt"
NOTEBOOK_PATH = RUN_DIR / "whisper_medium_lora_levantine_custom_streaming_5minckpt_run.ipynb"
SMOKE_NOTEBOOK_PATH = RUN_DIR / "whisper_medium_lora_levantine_custom_streaming_5minckpt_smoketest.ipynb"
EXECUTED_SMOKE_NOTEBOOK_PATH = RUN_DIR / "whisper_medium_lora_levantine_custom_streaming_5minckpt_smoketest_executed.ipynb"
...
LOGGER = logging.getLogger("whisper_medium_run")


@dataclass
class RunConfig:
    model_name: str = "openai/whisper-medium"
    run_id: str = "whisper_medium_levantine_custom_streaming_5minckpt"
...
def setup_logging() -> Path:
    ensure_run_layout()
    log_path = LOG_DIR / "whisper_medium_run.log"
...
def load_whisper_bundle(config: RunConfig, *, adapter_path: str | Path | None = None) -> WhisperBundle:
    ...
    model_root = RUN_DIR / "models" / "openai_whisper-medium"
...
class WhisperManifestDataset:
    ...
    def __getitem__(self, index: int) -> dict[str, Any]:
        ...
        labels = self.processor.tokenizer(normalize_arabic_text(row.get("text", ""))).input_ids
        sample = {

...
OUTPUT_ROOT = Path('/home/MohammadNabulsi/whisper/data_cleaned_text_omnilingual_v3')
...
def strip_placeholders_before_checks(text: Any) -> tuple[str, bool]:
    ...
    stripped = ANGLE_SPAN_RE.sub(' ', text)
...
def main() -> None:
    ...
    for shard_path in shard_paths:
        ...
        for row in rows:
            ...
            out_row['precheck_text_v3'] = precheck_text
            out_row['manual_normalized_transcript'] = normalized
            out_row['flag_removed_token_span_v3'] = removed_placeholder
            ...
            shard_counts['removed_token_span_v3'] += int(removed_placeholder)
    ...
    report = {
        ...
        'token_span_cleanup_terms': list(PLACEHOLDER_TERMS),
    ...
    with (reports_dir / 'per_dataset_report.csv').open('w', encoding='utf-8', newline='') as f:
        fieldnames = ['dataset', 'total', 'kept', 'dropped_contains_english', 'dropped_contains_number', 'dropped_audio_too_short', 'removed_token_span_v3', 'contains_bracket_token_original', 'contains_bracket_token_after_precheck']
        ...
        for dataset, cnt in sorted(per_dataset.items()):
            writer.writerow({
                ...
                'removed_token_span_v3': totals.get('removed_token_span_v3', 0),
    ...
    summary_lines = [
        ...
        f'Rows where bracket/token spans were stripped before checks: {totals.get("removed_token_span_v3", 0)}',

"""Resumable runner: zero-shot Omnilingual ASR (omniASR_LLM_300M) predictions + WER/CER
for one or all datasets discovered by datasets.discover_eval_targets().

Mirrors run_eval.py's Whisper driver so results are directly comparable:
...
Usage:
    python run_omni_eval.py --all
    python run_omni_eval.py --dataset masc_c_only --batch-size 8
    python run_omni_eval.py --dataset omnilingual_apc_full --limit 20   # smoke test
"""
...
from omni_predict import OmniPredictConfig, OmniPredictor  # noqa: E402

REPO_ROOT = Path(__file__).resolve().parents[2]
DEFAULT_OUT_DIR = REPO_ROOT / "outputs" / "omni_asr_llm_300m_zero_shot"
...
# Omnilingual language-conditioning token per dataset. apc_Arab = South Levantine Arabic
# (covers Palestinian and Jordanian dialects, our custom MASC/QASR test, and Layla); arb_Arab
# = Modern Standard Arabic (used for MASC, which was classifier-selected as Levantine but
# still MSA-register broadcast speech more often than not).
LANG_BY_DATASET = {
    "casablanca_palestinian": "apc_Arab",
    "casablanca_jordanian": "apc_Arab",
    "masc_c_only": "arb_Arab",
    "masc_qasr_custom_test": "apc_Arab",  # this IS our highest-confidence Levantine speaker selection
    "omnilingual_apc_full": "apc_Arab",
    "omnilingual_apc": "apc_Arab",
    "layla": "apc_Arab",
}
DEFAULT_LANG = "apc_Arab"
...
def setup_logger(log_path: Path) -> logging.Logger:
    ...
    logger = logging.getLogger(f"omni_zero_shot_eval.{log_path.stem}")
...
def run_dataset(
    ...
    lang = LANG_BY_DATASET.get(dataset_key, DEFAULT_LANG)

    done_uids = load_done_uids(predictions_path)
    logger.info("[%s] resume: %d rows already done (lang=%s)", dataset_key, len(done_uids), lang)
    ...
    with open(predictions_path, "a", encoding="utf-8") as out_f:
        ...
        def flush_batch():
            ...
            if not batch_uid:
                return
            t0 = time.time()
            hyps = predictor.predict(batch_audio, [lang] * len(batch_audio))
...
def main():
    ...
    ap.add_argument("--model-id", default="omniASR_LLM_300M")
    ...
    try:
        config = OmniPredictConfig(model_id=args.model_id, batch_size=args.batch_size)
        predictor = OmniPredictor(config)
        ...
        logger.exception("FATAL: run_omni_eval crashed")
        raise

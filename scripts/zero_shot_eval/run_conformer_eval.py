"""Resumable runner: zero-shot FastConformer-Hybrid (CTC) predictions + WER/CER for one
or all datasets discovered by datasets.discover_eval_targets().

Mirrors run_omni_eval.py / run_eval.py's driver so results are directly comparable:
- Predictions are appended to outputs/<run>/<dataset>/predictions.jsonl as soon as each
  batch finishes (flush + fsync). On restart, uids already present in that file are
  skipped, so a crash/kill loses at most one in-flight batch.
- Progress (rows/s, GPU memory) is logged every 2 minutes.
- Any unhandled exception is logged with a full traceback before the process exits, so
  a crash is always visible in run.log even without an external watchdog.

Usage:
    python run_conformer_eval.py --all
    python run_conformer_eval.py --dataset masc_c_only --batch-size 8
    python run_conformer_eval.py --dataset omnilingual_apc_full --limit 20   # smoke test
"""
...
# `python run_conformer_eval.py` puts this script's own directory at sys.path[0]
# automatically, before any code here runs. That directory has its own datasets.py (the
# discover_eval_targets helper below), which shadows the real HF `datasets` PyPI package
# for any plain `import datasets` / `from datasets import ...` done anywhere in this
# process -- including nemo.collections.asr's own internal
# `from datasets import concatenate_datasets`, which then crashes with "cannot import
# name 'concatenate_datasets' from 'datasets'" (resolved to this file instead of
# site-packages). Two fixes are both required, in this order:
#   1. Strip this directory from sys.path *before* importing nemo, so nemo resolves and
#      caches the REAL `datasets` package under sys.modules["datasets"]. (Restoring the
#      directory to sys.path afterwards is NOT enough by itself -- see point 2.)
#   2. Never go through the ambiguous bare name "datasets" for OUR OWN
#      discover_eval_targets access either: once nemo has cached the real package under
#      sys.modules["datasets"], a later `from datasets import discover_eval_targets`
#      reuses that same cached module (import lookups check sys.modules before sys.path)
#      and fails the opposite way. Load this directory's datasets.py directly by file
#      path under a private module name instead, which bypasses sys.modules entirely.
_SCRIPT_DIR = str(Path(__file__).resolve().parent)
sys.path = [p for p in sys.path if Path(p or ".").resolve() != Path(_SCRIPT_DIR).resolve()]
import nemo.collections.asr as _nemo_asr_preimport  # noqa: F401,E402
sys.path.insert(0, _SCRIPT_DIR)

from audio_io import decode_audio_cell  # noqa: E402
from conformer_predict import ConformerPredictConfig, ConformerPredictor  # noqa: E402
from evaluate import evaluate  # noqa: E402

_dspec = importlib.util.spec_from_file_location("_zero_shot_eval_datasets", str(Path(_SCRIPT_DIR) / "datasets.py"))
_dmod = importlib.util.module_from_spec(_dspec)
_dspec.loader.exec_module(_dmod)
discover_eval_targets = _dmod.discover_eval_targets

REPO_ROOT = Path(__file__).resolve().parents[2]
DEFAULT_OUT_DIR = REPO_ROOT / "outputs" / "conformer_ctc_zero_shot"
...
def setup_logger(log_path: Path) -> logging.Logger:
    ...
    logger = logging.getLogger(f"conformer_zero_shot_eval.{log_path.stem}")
...
def main():
    ...
    ap.add_argument("--model-id", default="nvidia/stt_ar_fastconformer_hybrid_large_pcd_v1.0")
    ...
    try:
        config = ConformerPredictConfig(model_id=args.model_id, batch_size=args.batch_size)
        predictor = ConformerPredictor(config)
        ...
        logger.exception("FATAL: run_conformer_eval crashed")
        raise

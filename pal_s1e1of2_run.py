...
RUN_TAG = PAL_RUN if PAL_STAGE2_EPOCHS == 1 else f"{PAL_RUN}_s2ep{PAL_STAGE2_EPOCHS}"
...
if STAGE1_SPLITS is None:
    ...
    # ---- STAGE 2: fresh LoRA on the merged model, 1 epoch on palTrain ----
    ...
    mlflow.end_run()
    s2_final = _final_adapter_dir(TRAIN_OUTS["stage2"], STAGE2_EPOCHS)
    try:
